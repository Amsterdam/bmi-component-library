/**
 * Taken from asc, the module does not export this util hook
 */
 ...
 */
const usePassPropsToChildren = (childrenProps: ReactNode, propsOrCallback: PropsWithoutRef<any> | Callback) => {
	const children = Children.map(childrenProps, (child, index) =>
		cloneElement(
			child as ReactElement<any>,
			typeof propsOrCallback === 'function' ? propsOrCallback(index) : propsOrCallback,
		),
	);
